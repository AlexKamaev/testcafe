...
export default class Driver extends serviceUtils.EventEmitter {
    ...
    _ensureClosedChildWindowWatcher () {
        ...
        this.checkClosedChildWindowIntervalId = nativeMethods.setInterval.call(window, () => {
            ...
            const childWindowClosedEventArgs = { setParentAsMaster: true };
            ...
            if (childWindowClosedEventArgs.setParentAsMaster)
    ...
    _closeWindowAndWait (childWindowToClose, msg) {
        const waitWindowForClose = this._createWaitForEventPromise(CHILD_WINDOW_CLOSED_EVENT, CHILD_WINDOW_CLOSED_EVENT_TIMEOUT, e => {
            e.setParentAsMaster = msg.closeSelf;
        });
    ...
    _onWindowCloseCommand (command) {
        ...
        const closeSelf                     = windowId === this.windowId;
        ...
        this._validateChildWindowCloseCommandExists(windowId, wnd)
            .then(response => {
                ...
                return sendMessageToDriver(new CloseWindowCommandMessage({ windowId, closeSelf }), wnd, WAIT_FOR_WINDOW_DRIVER_RESPONSE_TIMEOUT, CannotSwitchToWindowError);
            })
            .then(() => {

...
import { MouseDownStateController, MouseUpState } from './automation-states';
...
export default class MouseUpAutomation extends VisibleElementAutomation {
    private modifiers: {};
    private upState: MouseUpState;
    private downState: MouseDownStateController;
    private eventArgs: any;
    // public element: HTMLElement;

    public constructor (element: HTMLElement | null, mouseOptions: MouseOptions, mouseDownState: MouseDownStateController, eventArgs: { element: HTMLElement }) {
        super(element, mouseOptions);

        this.modifiers = mouseOptions.modifiers;

        this.upState = MouseUpState.from({ clickElement: null });

        this.eventArgs = eventArgs;

        this.downState = MouseDownStateController.from(mouseDownState);

        this.element = this.eventArgs?.element || this.element;
    ...
    public run (useStrictElementCheck: boolean): Promise<unknown> {
        ...
        if (!this.eventArgs) {
            promise = this._ensureElement(useStrictElementCheck, false)
        ...
        return promise
            .then(() => this._mouseup(this.eventArgs))
            .then(({ timeStamp }: { timeStamp: number }) => {
                this.options.timeStamp = timeStamp;

                return this._click(this.eventArgs);
            });
    }

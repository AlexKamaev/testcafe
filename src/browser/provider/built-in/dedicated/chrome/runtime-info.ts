...
export default class ChromeRuntimeInfo {
    ...
    protected async createTempProfile (proxyHostName: string): Promise<TempDirectory> {
        return await createTempProfile(proxyHostName);
    }

    public static async create (proxyHostName: string, configString: string): Promise<ChromeRuntimeInfo> {
        const runtimeInfo = new this(configString);

        if (!runtimeInfo.config.userProfile)
            runtimeInfo.tempProfileDir = await runtimeInfo.createTempProfile(proxyHostName);

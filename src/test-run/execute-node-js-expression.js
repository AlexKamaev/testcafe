...
function createErrorOptions (expression) {
    return {
...
export function executeNodeJsExpression (expression, testRun, options) {
    const context      = createExecutingContext(testRun, false, options);
    const errorOptions = createErrorOptions(expression);

    return runInNewContext(expression, context, errorOptions);
}

export async function executeAsyncNodeJsExpression (expression, testRun, callsite) {
    const proxy        = createExecutingContext(testRun, true);
    const errorOptions = createErrorOptions(expression);

    try {
        return await runInNewContext(wrapModule(expression), proxy, errorOptions)();
    }
    catch (err) {
